...
from shared import SumOfSquaresError, CrossEntropyError, DataSet, Instance
...
def initialize_instances():
    ...
    train = instances[:int(len(instances)*0.7)]
    test = instances[int(len(instances)*0.7):]
    
    cv = test[:int(len(test)*0.4)]
...
def train(oa, network, oaName, instances, cv, measure):
    ...
    for iteration in xrange(TRAINING_ITERATIONS):
        ...
        error = 0.00
        cv_error = 0.00
        shuffle = random.sample(xrange(len(instances)), 64)
        cv_shuffle = random.sample(xrange(len(cv)), 64)
